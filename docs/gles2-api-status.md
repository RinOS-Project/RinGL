...
| 2D texture targets, canonical `ALPHA`/`RGB`/`RGBA`/`LUMINANCE`/`LUMINANCE_ALPHA`, native RGB565/RGBA4/RGB5_A1, filters and wraps | P | `TEXTURE_2D` only. `RINGL_ETC1_RGB8_OES` is accepted only by the dedicated bounded compressed upload APIs and is expanded to RGB8; no cube map, other compressed formats, 3D, array, or immutable storage. |

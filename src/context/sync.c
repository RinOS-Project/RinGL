                                           ...
                                           void* pixels)
{
    ...
    if (packed_color_format(target.format)) {
        ...
        if (native_total_bytes > SIZE_MAX)
            return -1;
        native_pixels = ringl_context_alloc_staging(context, native_total_bytes);
    ...
    if (old_state == 0u ||
        ...
        submit_and_wait(context, command_list) != 0) {
        ringl_context_free_staging(context, native_pixels, native_total_bytes);
        return -1;
    }
                                    ...
                                    uint64_t tight_total_bytes)
{
    ...
    tight_pixels = ringl_context_alloc_staging(context, tight_total_bytes);
    ...
    if (clipped_width == (uint64_t)(uint32_t)width &&
        ...
        clipped_pixels = ringl_context_alloc_staging(context, clipped_total_bytes);
    ...
    ringl_context_free_staging(context, clipped_pixels, clipped_total_bytes);
    ringl_context_free_staging(context, tight_pixels, tight_total_bytes);
    return result;
}

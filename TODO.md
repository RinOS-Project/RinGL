...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Permit a second direct/finite-affine local `vec2` derived from the first, and lower the two declarations in source order before samples. Each non-direct local has its own RSH1 temporary quartet; a later call-local offset uses a separate quartet, preserving Float32 evaluation order. The combined eight-call ceiling is 109 instructions and 76 registers. Strict IR verifies the two local operations, the 8-call/16-resource ceiling, and third-local rejection; the RinGL→RinGPU→Aquamarine bridge samples the distinct blue texel selected only through `baseUv` then `sampleUv`.
  - [ ] Other nonconstant coordinate expressions, coordinates derived from more than two varyings or locals, other local vector expressions, more than two locals, different fragment expressions, more than eight calls, and broader varying-coordinate sampler paths remain unsupported.
...
- [x] Execute a level-zero `DEPTH_COMPONENT32F` texture through the existing bounded `sampler2D`/`texture2D()` profile. RinGL realizes D32 with `SAMPLED` usage; the normal RinGPU sampled-image contract accepts D32 as well as color, and the Aquamarine executor snapshots each depth texel as deterministic `(depth, 0, 0, 1)`. The focused RinGL→RinGPU→Aquamarine bridge reads the actual 0.5 depth value as red 128; sampling an image attached to the active FBO is rejected before transition/render-pass submission. D24S8 texture sampling remains outside this slice.
- [x] Map a level-zero `DEPTH24_STENCIL8`/`DEPTH_STENCIL`/`UNSIGNED_INT_24_8` texture to `DEPTH_STENCIL_ATTACHMENT`: convert WebGL packed 24/8 input to RinGPU's F32/S8 storage, preserve it through lazy upload, and use the existing D32S8 clear, depth, and separate-face stencil paths. D24S8 texture sampling remains outside this bounded slice.

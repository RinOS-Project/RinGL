...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Add the initial perspective `varying vec2` two-sampler profile: exactly two declared samplers are sampled once at the same varying coordinate and added. It emits two typed pairs and four scalar adds; strict IR and RinGL→RinGPU→Aquamarine integration tests verify the interpolated red + green yellow result.
  - [ ] Other nonconstant coordinate expressions, repeated or partially active varying-coordinate sampler chains, different fragment expressions, and broader varying-coordinate multi-sampler paths remain unsupported.

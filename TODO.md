...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Permit a single declared `sampler2D` to occur in one through eight finite constant-coordinate calls in the additive chain. Every call gets independent coordinate/sample/accumulator registers but references the same one typed RSH1 image/sampler pair; strict IR and RinGL→RinGPU→Aquamarine tests verify the two-call clamped-red draw. A second declaration left unused still fails before submission instead of fabricating an unused resource kind.
  - [ ] Repeated sampler use in a multi-declaration program when another declaration is unused requires active-resource reflection plus selective RinGPU bind-group construction; do not weaken RinGPU's invariant that every declared RSH1 resource has a type and use. Nonconstant coordinate expressions, different fragment expressions, and varying-coordinate multi-sampler paths also remain unsupported.
...
- [x] Map a level-zero `DEPTH_COMPONENT32F`/`DEPTH_COMPONENT`/`FLOAT` texture to `DEPTH_ATTACHMENT`: lazy D32 RinGPU image creation/upload, framebuffer completeness, depth clear, and all depth-tested draws use the same target path as a D32 renderbuffer. Texture sampling remains outside this bounded slice.
- [x] Map a level-zero `DEPTH24_STENCIL8`/`DEPTH_STENCIL`/`UNSIGNED_INT_24_8` texture to `DEPTH_STENCIL_ATTACHMENT`: convert WebGL packed 24/8 input to RinGPU's F32/S8 storage, preserve it through lazy upload, and use the existing D32S8 clear, depth, and separate-face stencil paths. Depth/depth-stencil texture sampling remains outside this bounded slice.

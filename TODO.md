...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Add a bounded three-`varying vec2` texture profile. Three sampler calls may select `firstUv`, `secondUv`, and `thirdUv` independently; the fragment RSH1 has six scalar inputs, and the third pair receives dedicated registers rather than aliasing either earlier pair. It also admits exactly one `mixedUv = firstUv +/- secondUv`, `firstUv +/- thirdUv`, or `secondUv +/- thirdUv` local alongside direct calls on the remaining pair. Strict IR checks all three locations, including a second/third-pair subtraction, and the selected local register pair. The RinGL→RinGPU→Aquamarine test executes a second/third-pair local with a direct first pair, ordinary color arithmetic for direct and offset-`UNSIGNED_BYTE` indexed triangle lists, strips, and fans; its indexed fan zero-divisor preflight leaves the target unchanged, and its short strip/fan draws are successful no-ops.
  - [ ] Other nonconstant coordinate expressions, coordinates derived from more than three varyings or locals, broader local expressions spanning three UV inputs, other local vector expressions, more than eight locals, shapes exceeding the RSH1 capacity budget, different fragment expressions, more than eight calls, and broader varying-coordinate sampler paths remain unsupported.
- [ ] Expand varying support beyond the initial `vec2` perspective-interpolated profile.
  ...
  - [x] Extend the RinGPU surface transport without changing its public compact clip-vertex V1 ABI: the native triangle-list route now carries six finite scalar varyings through six-plane clipping, perspective interpolation, fragment preflight, and submission for the bounded three-`vec2` texture profile. It accepts only the matching 10-scalar vertex/6-scalar fragment RSH1 interface; points, lines, strips, fans, arbitrary scalar widths, and general varying expressions remain unsupported on this V2 route.
...
- [ ] Implement stencil support after framebuffer/renderbuffer depth-stencil storage is in place.
  ...
  - [x] Add WebGL 1 `STENCIL_INDEX8` renderbuffers. RinGL reports the logical eight-bit stencil/no-depth format, accepts it only for `STENCIL_ATTACHMENT`, and realizes a D32S8 RinGPU target with its depth aspect kept logically inaccessible. Strict C11 FBO tests and the actual RinGL→RinGPU→Aquamarine test cover query, completeness, stencil clear/test, and rejection of its invalid use as `DEPTH_ATTACHMENT`.

...
typedef struct FakeBackend {
    ...
    uint32_t pipeline_topologies[5];
    uint32_t submissions;
    uint32_t indexed_draws;
    uint32_t indexed_formats[6];
    uint32_t indexed_counts[6];
    uint32_t vertex_draws;
    uint32_t vertex_counts[5];
    float clear[4];
} FakeBackend;
                              ...
                              const RinGLRinGpuDrawVerticesV1* draw)
{
    ...
    assert(backend->vertex_draws < 5u);
                             ...
                             const RinGLRinGpuDrawIndexedV1* draw)
{
    ...
    assert(backend->indexed_draws < 6u);
...
int main(void)
{
    ...
    assert(strcmp(commands,
                  "NTBECSRBDECSRBIECSRBIECSRBDECSRBIECSRBDECSRBIECSRBDECSRBIECSRBDECSRBIECSRTPCS") == 0);

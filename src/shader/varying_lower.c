...
static int extract_point_size_assignment(
    ...
    uint32_t* attribute_opcode, float* attribute_operand)
{
    char* match = NULL;
    ...
    if (source == NULL || has_point_size == NULL || point_size == NULL ||
        ...
        attribute_opcode == NULL || attribute_operand == NULL ||
        (uniform_count != 0u && uniforms == NULL)) {
        return 0;
    }
    *has_point_size = 0;
...
static int append_point_size_output(RinGLGlslLowerResult* result,
                                    ...
                                    uint32_t attribute_opcode,
                                    float attribute_operand)
{
    ...
    if (result == NULL || (uses_constant != 0 && !isfinite(point_size)) ||
        (attribute_opcode != 0u && !isfinite(attribute_operand)) ||
    ...
    if (header.magic != RINGL_RSH1_MAGIC ||
        ...
        (attribute_opcode != 0u &&
         (header.register_count > RINGL_RSH1_MAX_REGISTERS - 2u ||
          (attribute_opcode != RINGL_RSH1_OP_ADD_F32 &&
           attribute_opcode != RINGL_RSH1_OP_SUB_F32 &&
           attribute_opcode != RINGL_RSH1_OP_MUL_F32 &&
           attribute_opcode != RINGL_RSH1_OP_DIV_F32))) ||
        (uses_constant == 0 && source_register >= header.register_count) ||
        header.total_size != result->byte_size) {
        return 0;
    }
    additional_instructions = uses_constant != 0 ? 2u :
        attribute_opcode != 0u ? 3u : 1u;
    ...
    } else if (attribute_opcode != 0u) {
        memcpy(&point_size_bits, &attribute_operand, sizeof(point_size_bits));
        init_instruction(&instructions[instruction_count - 1u],
                         RINGL_RSH1_OP_CONST_F32);
        instructions[instruction_count - 1u].destination =
            (uint16_t)header.register_count;
        instructions[instruction_count - 1u].immediate = point_size_bits;
        init_instruction(&instructions[instruction_count], attribute_opcode);
        instructions[instruction_count].destination =
            (uint16_t)(header.register_count + 1u);
        instructions[instruction_count].source0 = source_register;
        instructions[instruction_count].source1 =
            (uint16_t)header.register_count;
        source_register = (uint16_t)(header.register_count + 1u);
                                        ...
                                        const float uniform_values[2])
{
    ...
    *coordinate_kind = **cursor == '+'
    ...
    if (consume_text(cursor, "vec2(")) {
        if (!parse_finite_float(cursor, offset_u) || !consume_text(cursor, ",") ||
...
static int parse_varying_texture_call(const char** cursor,
                                      ...
                                      const char* coordinate_uniform_name,
                                      ...
                                      VaryingTextureCall* call)
{
    ...
    if (coordinate_uniform_name != NULL && coordinate_uniform != NULL &&
        strcmp(coordinate, coordinate_uniform_name) == 0) {
        uint32_t coordinate_uniform_components[4];
        ...
        if (!read_identifier(cursor, secondary_coordinate,
            ...
            !sampler_index_for_name(coordinate_names, coordinate_count,
                                    secondary_coordinate, &coordinate_index) ||
            !parse_optional_read_swizzle(cursor, 2u, 2u,
        ...
        call->coordinate_kind = operator_kind == '+'
            ? RINGL_VARYING_TEXTURE_COORD_ADD_OFFSET
            : operator_kind == '-'
            ? RINGL_VARYING_TEXTURE_COORD_REVERSE_SUB_OFFSET
            : RINGL_VARYING_TEXTURE_COORD_MUL_SCALE;
    ...
    if (!sampler_index_for_name(coordinate_names, coordinate_count, coordinate,
                                &coordinate_index)) {
        return 0;
    }
    call->coordinate_input_location =
...
static int parse_varying_texture_local_coordinate(
    ...
    float* offset_v, const char* coordinate_uniform_name,
    const float coordinate_uniform[2])
{
    char source[64];
...
static int parse_varying_texture_local_coordinate_or_varying(
    ...
    const char* coordinate_uniform_name, const float coordinate_uniform[2])
{
    char source[64];
    char secondary[64];
    uint32_t coordinate_uniform_components[4];
    ...
    uint32_t uniform_count, RinGLGlslLowerResult* result)
{
    ...
    char coordinate_uniform_name[64] = {0};
    ...
    uint32_t has_coordinate_uniform = 0u;
    ...
    uint32_t component;
    float color[4] = {0.0f};
    float coordinate_uniform[2] = {0.0f, 0.0f};
    size_t total;
    ...
    while (strncmp(cursor, "uniformsampler2D", strlen("uniformsampler2D")) ==
           ...
           strncmp(cursor, "uniformvec2", strlen("uniformvec2")) == 0) {
        if (strncmp(cursor, "uniformsampler2D",
            ...
            ++sampler_count;
        } else if (strncmp(cursor, "uniformvec4", strlen("uniformvec4")) ==
                   0) {
            ...
            if (has_coordinate_uniform ||
                !consume_text(&cursor, "uniformvec2") ||
                !read_identifier(&cursor, coordinate_uniform_name,
                                 sizeof(coordinate_uniform_name)) ||
                !consume_text(&cursor, ";")) {
                return 1;
            }
            has_coordinate_uniform = 1u;
        }
    }
    ...
    if (has_coordinate_uniform) {
        for (call_index = 0u; call_index < uniform_count; ++call_index) {
            if (uniforms[call_index].name == NULL ||
                strcmp(uniforms[call_index].name,
                       coordinate_uniform_name) != 0) {
                continue;
            }
            if (uniforms[call_index].type != RINGL_FLOAT_VEC2 ||
                !isfinite(uniforms[call_index].values[0]) ||
                !isfinite(uniforms[call_index].values[1])) {
                return 1;
            }
            memcpy(coordinate_uniform, uniforms[call_index].values,
                   sizeof(coordinate_uniform));
            break;
        }
    }
        ...
        while (strncmp(cursor, "vec2", strlen("vec2")) == 0) {
            if (local_coordinate_count ==
                ...
                !parse_varying_texture_local_coordinate(
                    ...
                    has_coordinate_uniform ? coordinate_uniform_name : NULL,
                    has_coordinate_uniform ? coordinate_uniform : NULL)) {
                return 1;
            }
    ...
    RinGLGlslLowerResult* result)
{
    ...
    uint32_t point_size_attribute_opcode;
    float point_size_attribute_operand;
    ...
    if (shader_type != RINGL_VERTEX_SHADER) {
        ...
        point_size_attribute_opcode = 0u;
        point_size_attribute_operand = 0.0f;
    }

...
| `glUniform1f`, `glUniform1fv`, `glUniform2f`, `glUniform2fv`, `glUniform3f`, `glUniform3fv`, `glUniform4f`, `glUniform4fv` | P | Finite scalar/vector values realize a program-owned RinGPU module only for the stage that reads the linked `uniform float`/`vec2`/`vec3`/`vec4` source, preserving an opposite-stage sampler/varying module; an embedding may map each `*fv` form only when its single non-array value is complete. |

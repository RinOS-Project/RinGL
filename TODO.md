...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Add a bounded three-`varying vec2` texture profile. Three sampler calls may select `firstUv`, `secondUv`, and `thirdUv` independently; the fragment RSH1 has six scalar inputs, and the third pair receives dedicated registers rather than aliasing either earlier pair. It also admits exactly one `mixedUv = firstUv +/- secondUv` local alongside direct `thirdUv` calls. Strict IR checks all three locations and the selected local register pair. The RinGL→RinGPU→Aquamarine test executes ordinary color arithmetic for direct and offset-`UNSIGNED_BYTE` indexed triangle lists, strips, and fans; its indexed fan zero-divisor preflight leaves the target unchanged, and its short strip/fan draws are successful no-ops.

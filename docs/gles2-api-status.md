...
| `glGetBufferParameteriv` | B | `ringl_get_buffer_parameteriv_bounded` exposes the complete GLES 2.0 buffer-query set (`BUFFER_SIZE` and `BUFFER_USAGE`) after complete target/binding/pname/span validation; values that cannot fit the represented `GLint` range are rejected before publication. |
| `glGetError` | B | `ringl_get_error`. |
| `glGetFramebufferAttachmentParameteriv` | P | Bounded `ringl_get_framebuffer_attachment_parameteriv_bounded` exposes object type/name, texture level, the 2D-only cube-face sentinel, standard/RinGL color encoding, component type, and logical RED/GREEN/BLUE/ALPHA/DEPTH/STENCIL sizes for represented default/custom-FBO attachments. Default object pnames report `NONE`/zero and metadata comes from the configured physical/aspect descriptor; the versioned attachment record exposes the same default `NONE`/zero identity. Complete one-element output spans are validated before publication. Cube, multisample, and unsupported-format semantics remain unavailable. |
...
| `glGetProgramiv`, `glGetShaderiv` | P | Bounded `ringl_get_program_parameteriv_bounded` and `ringl_get_shader_parameteriv_bounded` expose represented link/validate/count, active-name maxima, log/source-length, and compile/type pnames through complete one-element output spans. Delete-status and other unsupported pnames remain unavailable. |
...
| `glPixelStorei` | B | `PACK_ALIGNMENT` and `UNPACK_ALIGNMENT` implement the complete GLES 2.0 entry-point state for values 1, 2, 4, and 8; the selected alignment is consumed by bounded upload/readback row-layout validation. |
...
| `glTexParameteri` | B | The complete GLES 2.0 sampler-parameter set (`MIN_FILTER`, `MAG_FILTER`, `WRAP_S`, and `WRAP_T`) is validated for supported 2D/cube targets and mapped to RinGPU sampler state. Extension-only anisotropy remains on its separately gated entry point. |

...
typedef struct FakeBackend {
    ...
    uint32_t pipeline_topologies[4];
    uint32_t submissions;
    uint32_t indexed_draws;
    uint32_t indexed_formats[5];
    uint32_t indexed_counts[5];
    uint32_t vertex_draws;
    uint32_t vertex_counts[4];
    float clear[4];
} FakeBackend;
                              ...
                              const RinGLRinGpuDrawVerticesV1* draw)
{
    ...
    assert(backend->vertex_draws < 4u);
                             ...
                             const RinGLRinGpuDrawIndexedV1* draw)
{
    ...
    assert(backend->indexed_draws < 5u);
...
int main(void)
{
    ...
    assert(backend.submissions == 11u);
    assert(backend.vertex_draws == 4u);
    ...
    assert(strcmp(commands,
                  "NTBECSRBDECSRBIECSRBIECSRBDECSRBIECSRBDECSRBIECSRBDECSRBIECSRTPCS") == 0);

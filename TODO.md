...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Permit a third direct/finite-affine local `vec2` derived from the second and lower all declarations in source order before sampling. The all-affine eight-call ceiling is 113 instructions and 80 registers, inside the RSH1 128-instruction/96-register limits. Strict IR verifies that all three operations consume their predecessor's result; the actual RinGL→RinGPU→Aquamarine draw reaches the blue texel only after the third local subtracts `vec2(0.0, 0.5)`.
  - [ ] Other nonconstant coordinate expressions, coordinates derived from more than two varyings or locals, other local vector expressions, more than three locals, different fragment expressions, more than eight calls, and broader varying-coordinate sampler paths remain unsupported.

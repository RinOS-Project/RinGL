...
int main(void)
{
    ...
    assert(ringl_validate_vertex_fetch(context, 0u, 3u, &layout) == 0);
    {
        ...
        uint64_t cache_misses = context->vertex_validation_cache_misses;
        uint64_t generation = context->vertex_validation_generation;

        assert(ringl_validate_vertex_fetch(context, 0u, 3u, &layout) == 0);
        assert(context->vertex_validation_cache_hits == cache_hits + 1u);
        assert(context->vertex_validation_cache_misses == cache_misses);
        ...
        cache_misses = context->vertex_validation_cache_misses;
        assert(ringl_validate_vertex_fetch(context, 0u, 3u, &layout) == 0);
        assert(context->vertex_validation_cache_hits == cache_hits);
        assert(context->vertex_validation_cache_misses == cache_misses + 1u);
    }

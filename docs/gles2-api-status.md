...
| `glGetUniformfv` | P | `ringl_get_uniform_{1,2,3,4}f` for linked `float`/`vec2`/`vec3`/`vec4` locations (including one element of a bounded scalar float array), and `ringl_get_uniform_matrix{2,3,4}f` for bounded vertex square-matrix profiles. |
| `glGetUniformLocation` | P | Linked `sampler2D`, bounded scalar float arrays, scalar/vector float, signed integer, scalar/vector Boolean, and bounded vertex `mat2`/`mat3`/`mat4` uniforms only. |
...
| `glUniform1i` | P | Linked `sampler2D` (including one bounded sampler-array element), scalar `int`, or scalar `bool` location; Boolean writes normalize zero/non-zero to `0`/`1`; integer/Boolean arrays are unavailable. |
| `glUniform1f`, `glUniform1fv`, `glUniform2f`, `glUniform2fv`, `glUniform3f`, `glUniform3fv`, `glUniform4f`, `glUniform4fv` | P | `glUniform1fv` updates one finite scalar or a complete contiguous range of one bounded scalar `float` array; its RSH1 replacements are published atomically. Other finite scalar/vector values realize a program-owned RinGPU module only for stages that declare the updated uniform name, preserving independent sampler/varying modules. One fragment `uniform vec2` can be the right-hand finite operand of bounded varying-coordinate `+`, `-`, `*`, or `/` texture forms and either operand of one direct sampler-coordinate `+`, `-`, `*`, or `/` form. RSH1 preserves `uniform - coordinate` and `uniform / coordinate` source order. A full-width `xy`/`yx`, `rg`/`gr`, or `st`/`ts` selector permutes its two atomically rematerialized fragment RSH1 constants before the native RinGPU draw. A zero uniform or varying divisor is rejected by backend preflight before target publication; zero literal divisors fail shader lowering. Uniform-led local coordinate initialization remains unsupported. The bounded vertex-colored texture material may sample one UV/image pair, or add two pairs, then apply a `vec4` tint and scalar opacity in RSH1 (`(texture2D(...) + texture2D(...)) * vertexColor * tint * opacity`); non-scalar-vector array updates remain unavailable. |
| `glUniform1iv` | P | One linked scalar `sampler2D`/`int`/`bool` value or a complete contiguous range inside one bounded sampler array. RinGL validates the complete range before mutating texture units; dynamic GLSL indices and integer/Boolean/vector/matrix arrays are unavailable. |

...
| 2D texture targets, canonical `ALPHA`/`RGB`/`RGBA`/`LUMINANCE`/`LUMINANCE_ALPHA`, native RGB565/RGBA4/RGB5_A1, filters and wraps | P | `TEXTURE_2D` only. ETC1 and the four S3TC DXT formats are accepted only by the dedicated bounded compressed upload APIs and are expanded to RGB/RGBA; no cube map, other compressed formats, 3D, array, or immutable storage. |

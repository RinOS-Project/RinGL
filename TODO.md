...
- [x] Map a level-zero `DEPTH_COMPONENT32F`/`DEPTH_COMPONENT`/`FLOAT` texture to `DEPTH_ATTACHMENT`: lazy D32 RinGPU image creation/upload, framebuffer completeness, depth clear, and all depth-tested draws use the same target path as a D32 renderbuffer. D32S8 depth-stencil textures and depth-texture sampling remain outside this bounded slice.

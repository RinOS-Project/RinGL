...
| `glUniform1i` | P | Linked `sampler2D` (including one bounded sampler-array element), scalar `int`, or scalar `bool` location; Boolean writes normalize zero/non-zero to `0`/`1`; numeric/Boolean arrays are unavailable. |
...
| `glUniform1iv` | P | One linked scalar `sampler2D`/`int`/`bool` value or a complete contiguous range inside one bounded sampler array. RinGL validates the complete range before mutating texture units; dynamic GLSL indices and numeric/Boolean arrays are unavailable. |

...
typedef struct FakeBackend {
    ...
    uint32_t pipeline_topologies[3];
    uint32_t submissions;
    uint32_t indexed_draws;
    uint32_t indexed_formats[4];
    uint32_t indexed_counts[4];
    uint32_t vertex_draws;
    uint32_t vertex_counts[3];
    float clear[4];
} FakeBackend;
                              ...
                              const RinGLRinGpuDrawVerticesV1* draw)
{
    ...
    assert(backend->vertex_draws < 3u);
                             ...
                             const RinGLRinGpuDrawIndexedV1* draw)
{
    ...
    assert(backend->indexed_draws < 4u);
...
int main(void)
{
    ...
    assert(strcmp(commands,
                  "NTBECSRBDECSRBIECSRBIECSRBDECSRBIECSRBDECSRBIECSRTPCS") == 0);

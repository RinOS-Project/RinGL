...
| `glUniform1f`, `glUniform1fv`, `glUniform2f`, `glUniform2fv`, `glUniform3f`, `glUniform3fv`, `glUniform4f`, `glUniform4fv` | P | Finite scalar/vector values realize a program-owned RinGPU module only for stages that declare the updated uniform name, preserving independent sampler/varying modules. One fragment `uniform vec2` can be the right-hand finite operand of the bounded varying-coordinate `+`, `-`, `*`, or `/` texture forms, and can lead one direct sampler-coordinate `+`, `-`, or `*` form. A left-hand subtraction emits RSH1 `uniform - coordinate` source order. A full-width `xy`/`yx`, `rg`/`gr`, or `st`/`ts` selector permutes its two atomically rematerialized fragment RSH1 constants before the native RinGPU draw. A zero uniform divisor is rejected by backend preflight before target publication; zero literal divisors fail shader lowering. Uniform-led division and uniform-led local coordinate initialization remain unsupported. The bounded vertex-colored texture material may sample one UV/image pair, or add two pairs, then apply a `vec4` tint and scalar opacity in RSH1 (`(texture2D(...) + texture2D(...)) * vertexColor * tint * opacity`); an embedding may map each `*fv` form only when its single non-array value is complete. |

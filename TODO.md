...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Execute bounded `uniform sampler2D name[N]` arrays (`1 <= N <= 8`, including all sampler elements) when every `texture2D(name[index], ...)` index is a decimal compile-time constant in range. RinGL expands the linked sampler locations as `name[0]` through `name[N-1]`, aliases `getUniformLocation("name")` to element zero, reports one active uniform with `name[0]` and its array size, and maps the selected element's declaration index to a real RSH1 image/sampler pair. `ringl_uniform_1iv()` updates only a contiguous in-array range after validating it in full; the Ladybird overload dispatches its complete `Int32List`, and the actual bridge draws the blue texture selected by `palette[1]` on texture unit one. Dynamic/out-of-range indices reject before module publication.
  - [ ] GLSL ES texture semantics outside the generic accepted expression grammar remain unsupported: dynamic sampler indexing, cube/3D/array/LOD/gradient shadow forms, non-sampler uniform arrays, loops/functions, storage side effects, unsupported control flow/types, and modules exceeding the 128-instruction/96-register RSH1 ceiling fail before publication. This does not claim complete GLSL ES/WebGL texture conformance.
...
- [x] Expose per-attachment logical sRGB encoding and component type to the
  WebGL embedding. The `*_at` framebuffer queries read RinGL's live
  `COLOR_ATTACHMENTi` state, gate nonzero slots behind
  `WEBGL_draw_buffers`, and leave output storage untouched on validation or
  object-lookup failure; `COLOR_ATTACHMENT0` shorthands remain compatible.

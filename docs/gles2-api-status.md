...
| `glGetProgramiv`, `glGetShaderiv` | P | Bounded `ringl_get_program_parameteriv_bounded` and `ringl_get_shader_parameteriv_bounded` expose represented link/validate/count and compile/type pnames through complete one-element output spans. Delete-status, info/source-length, and other unsupported pnames remain unavailable. |

...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Generalize the constant-coordinate additive profile to one through eight declared samplers: each sampler must occur exactly once in a left-to-right `+` chain, and lowering emits the complete declaration-ordered image/sampler pair table. The formulaic RSH1 layout consumes at most 85 instructions and 80 registers, so the RinGL ceiling is raised to 96 while remaining below RinGPU's public 256-register limit. Strict IR tests preserve the one/two-sampler bytecode layouts, reject a duplicated sampler rather than treating it as an omitted binding, inspect a three-sampler reverse declaration-order chain, and compile/lower all eight samplers; the RinGPU/Aquamarine bridge renders reverse-order red + green + blue and the full eight-sampler/16-resource ceiling as white via direct and indexed draws.
  - [ ] Repeated sampler use, nonconstant coordinate expressions, different fragment expressions, and varying-coordinate multi-sampler paths remain unsupported.

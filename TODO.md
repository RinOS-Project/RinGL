...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Lower bounded varying-coordinate texture color operations with finite `vec4` literals: `+`, `-`, `*`, or nonzero `/` after one sample, or after a one-through-eight sample additive chain only when explicitly parenthesized. RinGL emits four `CONST_F32` plus component-wise RSH1 operations after the live RinGPU samples/adds and rejects layouts beyond the RSH1 instruction/register ceilings or any zero division component. The finite literal may also lead one sample or a parenthesized additive chain for `+`, `-`, or `*`; operand order is preserved for subtraction. A leading `/` is rejected because a sampled divisor cannot be proven nonzero while lowering. Strict IR verifies every admitted operation, reverse subtraction, and rejection cases; the actual bridge subtracts two distinct sampled images from a leading color before readback. Unparenthesized multi-call precedence and broader fragment expressions remain unsupported.

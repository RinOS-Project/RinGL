...
| GLSL fixed stage-output selectors | B | Vertex `gl_Position` and fragment `gl_FragColor` accept non-overlapping writable `xyzw`/`rgba`/`stpq` selectors. RinGL emits exact scalar RSH1 output stores in source order and requires all four clip/color components before RinGPU module publication; a partial selector output fails linking, with no default-value or embedding-renderer fallback. `gl_PointSize`, `gl_FragDepthEXT`, and indexed `gl_FragData` retain their existing whole-output forms. |

...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Permit the bounded two-varying local result to feed up to five further direct/finite-affine locals, within the existing six-local limit: `mixedUv = firstUv +/- secondUv; sampleUv = mixedUv + vec2(...)` (and the other finite `-`, `*`, `/` operations). The combine result is retained in RSH1 registers and each later affine stage reads its predecessor in source order. Strict IR verifies the result/register chain; the actual bridge samples a fourth 4×4 texel reachable only after the two-varying combine and local offset.
  - [x] Lower bounded varying-coordinate texture tint expressions with finite literals: one call as `texture2D(sampler, coordinate) * vec4(r, g, b, a)`, or one-through-eight calls only when their additive chain is explicitly parenthesized before the tint. RinGL emits four `CONST_F32` plus component-wise `MUL_F32` instructions after the live RinGPU samples/adds and rejects layouts beyond the RSH1 instruction/register ceilings. Strict IR verifies one/two-sampler layouts and the actual bridge binds two distinct images, adds them, and tints the result before readback. Unparenthesized multi-call precedence and broader fragment expressions remain unsupported.
  - [ ] Other nonconstant coordinate expressions, coordinates derived from more than two varyings or locals, other local vector expressions, more than six locals, different fragment expressions, more than eight calls, and broader varying-coordinate sampler paths remain unsupported.

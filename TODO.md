...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Lower one bounded varying-coordinate texture tint expression, `gl_FragColor = texture2D(sampler, coordinate) * vec4(r, g, b, a)`, for finite literals and exactly one texture call. RinGL emits four `CONST_F32` plus component-wise `MUL_F32` instructions after the live RinGPU sample. Strict IR verifies each constant/register pair, while the actual bridge tints the result of a two-varying local-coordinate chain before readback. Multi-call tint precedence and broader fragment expressions remain unsupported.

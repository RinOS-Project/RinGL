...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [ ] GLSL ES texture semantics outside the generic accepted expression grammar remain unsupported: dynamic sampler/float indexing, cube/3D/array/LOD/gradient shadow forms, integer/Boolean/vector/matrix uniform arrays, loops/functions, storage side effects, unsupported control flow/types, and modules exceeding the 128-instruction/96-register RSH1 ceiling fail before publication. This does not claim complete GLSL ES/WebGL texture conformance.

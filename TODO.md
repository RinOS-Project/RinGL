...
- [x] Keep `DEPTH_ATTACHMENT` and `STENCIL_ATTACHMENT` as independent custom-FBO slots and submit distinct D32/S8 renderbuffers through the versioned three-target RinGPU render pass. The combined attachment query returns an object only for an identical shared attachment; otherwise depth and stencil queries return their separate owners. The actual RinGL→RinGPU→Aquamarine test verifies completeness, queries, simultaneous depth/stencil draw, depth-fail stencil update, and RGBA readback.
  ...
  - [x] Expose bounded GLES attachment component metadata for the represented FBO profile. `COMPONENT_TYPE`, logical RED/GREEN/BLUE/ALPHA/DEPTH/STENCIL sizes, and both standard and RinGL color-encoding pnames are failure-atomic for 2D textures and renderbuffers; depth/stencil aspect queries report only the logically attached plane. Unattached slots return zero metadata, and default-framebuffer metadata now comes from its configured physical format and explicit logical aspects; its versioned attachment record reports the standard NONE/zero object sentinel. Cube, multisample, and unsupported formats remain outside the profile.
  - [x] Saturate the signed stencil reference and clear value to the represented 8-bit plane. Negative values now become zero and values above 255 become 255 instead of wrapping through an unsigned mask; state and clear snapshots preserve the GLES-visible value. Remaining attachment-format, multisample, and conformance semantics stay under the parent item.
...
- [ ] Close GLES 2.0 semantic gaps found by conformance-style tests.
  ...
  - [x] Add bounded `glGetShaderiv`/`glGetProgramiv` adapters for the status,
    ...
    spans and existing versioned records. Shader and currently bound program
    `DELETE_STATUS` now report their pending bits while the object remains
    retained; unrepresented pnames remain unavailable rather than returning
    fabricated values.
  ...
  - [x] Preserve currently bound-program query lifetime for `DELETE_STATUS`:
    `glDeleteProgram` marks the program pending, makes `glIsProgram` false,
    and leaves its executable/query state available until `useProgram(0)` or a

...
- [ ] Expand texture expressions beyond the initial constant/varying-coordinate one-sampler slice.
  ...
  - [x] Accept one declared, unswizzled fragment `uniform vec2` as the right operand of bounded coordinate `+`, `-`, or `*` operations. Direct sampler calls and the existing source-order local-coordinate chains materialize the finite pair as two RSH1 Float32 constants; `ringl_uniform_2f()` atomically publishes the replacement fragment module before the native RinGPU pipeline/draw. Strict IR verifies the zero-default module and the native textured-draw test verifies the updated constant bits, real UV varyings, image/sampler binding, and draw submission. Uniform division, uniform selectors, and general vector-coordinate expressions remain unsupported.

...
`RINGL_POINTS`, `RINGL_LINES`, and `RINGL_LINE_STRIP` map to RinGPU's native
point-list, line-list, and line-strip topologies. The primitive is part of
...
paths before they reach the embedding; an incomplete line-list pair and a
line strip with fewer than two vertices are successful no-ops. Line loops,
triangle strips, and fans remain unsupported in this bounded primitive slice.
